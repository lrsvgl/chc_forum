...
class tx_chcforum_display extends tx_chcforum_pi1
{
	...
	function tx_chcforum_display($cObj)
	{
		...
		switch ($this->flag) {
			...
			case 'rate':
				if (($this->fconf['allow_rating'] == true) || (($this->fconf['allow_rating'] == 'logged_in_user') && ($this->user->uid))) $this->user->rate_post($this->rateSelect, $this->ratePostUID);
				break;
...
//			}
	...
	function cwt_message()
	{
		...
		switch ($this->action) {
			case 'getviewmessagesnew':
				...
				$recipient = t3lib_div::makeInstance("tx_chcforum_author",$this->recipient_uid, $this->cOb);
	...
	function user_list()
	{
		...
		$fields = 'uid';
		$table = 'fe_users';
		$where = 'deleted != 1 AND disable != 1 AND uid IS NOT NULL AND pid=\'' . $pid . '\'';
		...
		if ($this->conf['cwtCommunityIntegrated'] == true) {
			...
			$tmpl = t3lib_div::makeInstance("tx_chcforum_tpower",$this->tmpl_path . 'user_list.tpl');
			$tmpl->prepare();
	...
	{
		// hide Sub toolbar if no user logged in
		...
		if ($this->fconf['mailer_disable'] == false && ($this->view == 'single_thread' or $this->view == 'single_conf')) {
			...
			switch ($this->view) {
				case 'single_thread':
					...
					$watch_arr['watch_alt'] = tx_chcforum_shared::lang('alt_watch_thread');
				...
				case 'single_conf':
					...
					$watch_arr['watch_alt'] = tx_chcforum_shared::lang('alt_watch_conference');
