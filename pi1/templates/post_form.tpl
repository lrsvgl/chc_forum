<!-- chc_forum Template post_form.tpl -->
<form action="{action}" enctype="multipart/form-data" id="post" name="post" method="post" onsubmit="return checkForm(this)" class="tx-chcforum-pi1-postForm">
    ...
    <fieldset id="formTop">
        ...
        <label for="name">{label_name}</label> {name} {username}
        <br style="clear: both"/>
        <label for="email">{label_email}</label> {email}
        <br style="clear: both"/>
        <label for="subject">{label_subject}</label>
        <input type="text" name="subject" style="width: 80%" value="{subject}"/><br style="clear: both"/>
    </fieldset>
    ...
    <fieldset id="formBottom">
        ...
        <!-- START BLOCK : cancel -->
        <div class="tx-chcforum-pi1-formBtn">
            <input type="submit" name="cancel" value="{cancel}" onmouseover="this.className='tx-chcforum-pi1-formBtnHov'" onmouseout="this.className='tx-chcforum-pi1-formBtn'" onclick="return(confirm('{cancel_confirm}'))"/></div>
        <!-- END BLOCK : cancel -->
    </fieldset>
</form>