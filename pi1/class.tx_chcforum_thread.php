	  ...
	  * Thread object constructor. Constructs the object from a DB record.
		...
		*/	 
		function tx_chcforum_thread ($thread_id = false, $cObj) {
			...
			$addWhere = 'uid='.$GLOBALS['TYPO3_DB']->fullQuoteStr($thread_id,$table);
			$fields = '*';
			$limit = '1';
	 ...
	 /**
		...
		*/
		function return_thread_author_name() {
			...
			$post = t3lib_div::makeInstance("tx_chcforum_post",$this->thread_firstpostid, $this->cObj);
			if ($post->post_author) {
				$author =  t3lib_div::makeInstance("tx_chcforum_author",$post->post_author, $this->cObj);
			...
			// stored when the thread was created.
		  		...
		  		$this->author = t3lib_div::makeInstance("tx_chcforum_author",$this->thread_author, $this->cObj);
	 ...
	 /**
		...
		*/
		...
		function generate_thread_row_data($recent_post_id) {
			$recent_post = t3lib_div::makeInstance("tx_chcforum_post",$recent_post_id, $this->cObj);
		...
		function cache_thread_row_data($output_array, $post_id) {
			//debug($output_array,'output array');
			$data_arr['cached_last_post_info'] = substr(serialize($output_array),0,255);
			$data_arr['cached_last_post_id'] = $post_id;
			$table = 'tx_chcforum_thread';
			$where = 'uid='.$GLOBALS['TYPO3_DB']->fullQuoteStr($this->uid,$table);
			$GLOBALS['TYPO3_DB']->exec_UPDATEquery($table,$where,$data_arr);
			return true;
		}
