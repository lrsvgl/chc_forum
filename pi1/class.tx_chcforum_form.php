...
#		var $lorem_forum_count = 1;
	  ...
	  */
		function tx_chcforum_form($display_object) {
			...
			if ($this->flag == "quote" && $this->post_uid) {
				$quoted_post = t3lib_div::makeInstance("tx_chcforum_post",$this->post_uid, $this->cObj);
			...
			if ($this->view == "edit_post" && $this->post_uid && $this->user->can_edit_post($this->post_uid) == true) {
				$this->message = t3lib_div::makeInstance("tx_chcforum_message",$this->cObj, tx_chcforum_shared::lang('edit_msg'), 'message');
				$edit_post = t3lib_div::makeInstance("tx_chcforum_post",$this->post_uid, $this->cObj);
				if (empty($this->subject)) $this->subject = $edit_post->post_subject;
				if (empty($this->text)) $this->text = $edit_post->post_text;
				$this->author = t3lib_div::makeInstance("tx_chcforum_author",$edit_post->post_author, $this->cObj);
				if ($this->author->uid) {
    ...
    */
	 ...
	 /**
		...
		*/
		function preview() {
			$preview_post = t3lib_div::makeInstance("tx_chcforum_post",'', $this->cObj, true);
			...
			$preview_message = t3lib_div::makeInstance("tx_chcforum_message",$this->cObj, tx_chcforum_shared::lang('preview_message'), 'message');
	 ...
	 /** 
		...
		*/
		function display_form() {
			...
			if ($this->conf_uid && $this->user->can_write_conf($this->conf_uid) == true) {
				...
				if (!$this->tmpl_path) $this->tmpl_path = tx_chcforum_shared::setTemplatePath();
					$tmpl = t3lib_div::makeInstance("tx_chcforum_tpower",$this->tmpl_path.'post_form.tpl');
				$tmpl->prepare();
				$tmpl->assign($this->label_array);
		...
		*/
		function submit($update = false) {
			...
			switch ($this->where) {
				...
				if ($this->fe_user_uid && $this->cat_uid && $this->conf_uid) {
					$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid', $table, 
										      ...
										      " AND post_subject=".$GLOBALS['TYPO3_DB']->fullQuoteStr($this->subject,$table).
										      " AND deleted=0".
				...
				if (!$this->lorem_forum) {
						$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('uid', $table, 
									      	...
									      	" AND post_author= ".$this->fe_user_uid.
									      	" AND post_text= ".$GLOBALS['TYPO3_DB']->fullQuoteStr($this->text,$table).
 									      	" AND tstamp>".(time()-60*5));
					if ($GLOBALS['TYPO3_DB']->sql_num_rows($res)>0) {
				  	break;
					}
			...
			$this->message = t3lib_div::makeInstance("tx_chcforum_message",$this->cObj, tx_chcforum_shared::lang('form_submitted'), 'message');
