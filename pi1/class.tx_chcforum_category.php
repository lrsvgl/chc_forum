		...
		*/
		function tx_chcforum_category ($cat_id = false, $cObj) {
			...
			if (!$cat_id) {
				...
				$addWhere = 'uid='.$GLOBALS['TYPO3_DB']->fullQuoteStr($cat_id,$table);
				$fields = '*';
				$limit = '1';
	  ...
	  */
		function get_confs () {
			...
			if ($this->uid) {
				...
				$addWhere = 'cat_id='.$GLOBALS['TYPO3_DB']->fullQuoteStr($this->uid,$table);
		...
		function return_conf_titles() {
			...
			if (is_array($confs)) {
				foreach ($confs as $k => $uid) {
					$conf = t3lib_div::makeInstance("tx_chcforum_conference",$uid, $this->cObj);
