		...
		*/		
		function return_most_recent_thread_id () {
			if ($this->uid) {
				...
				$addWhere = 'tx_chcforum_post.conference_id='.$GLOBALS['TYPO3_DB']->fullQuoteStr($this->uid,$table);
	  ...
	  * Returns the total number of posts in a conference.
		...
		*/
		function return_post_count () {
			...
			if ($this->user->show_hidden == 1) {
				...
				$addWhere = 'tx_chcforum_post.conference_id='.$GLOBALS['TYPO3_DB']->fullQuoteStr($this->uid,$table).' AND tx_chcforum_thread.hidden=0';				
				$where = tx_chcforum_shared::buildWhere($table,$addWhere);
		...
		function return_post_ids_since ($tstamp, $show_hidden = false, $conf_id_arr = false, $return_thread_ids = false) {
			...
			$addWhere.= 'AND crdate > '.$GLOBALS['TYPO3_DB']->fullQuoteStr($tstamp,$table);
		...
		*/		
		function return_all_thread_ids ($show_hidden = false) {
			...
			$now = time();
			$fields = '
				DISTINCT tx_chcforum_thread.uid as uid,
				tx_chcforum_post.hidden as posthidden,
				tx_chcforum_thread.crdate,
				tx_chcforum_thread.tstamp,
				thread_attribute
			';
			...
			$addWhere = '
				tx_chcforum_thread.conference_id='.$GLOBALS['TYPO3_DB']->fullQuoteStr($this->uid,$table).'
				AND
				(tx_chcforum_thread.endtime >= \''.$now.'\' OR tx_chcforum_thread.endtime <= 0)
			';
			
			// Hide Threads if neccessary
			if ($this->user->show_hidden == false) $addWhere .= 'AND tx_chcforum_post.hidden = 0';
		...
		*/
		function return_cat_name () {;
			$cat = t3lib_div::makeInstance("tx_chcforum_category",$this->cat_id);
