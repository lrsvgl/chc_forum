		...
		*/
		function profile_form ($cwt_version = false) {
			...
			if ($this->fconf['mailer_disable'] != true) {
				...
				// output category-conference-tree for selectable conferences
				$confs = $this->get_viewable_confs();
				if ($confs) {
					$selected_confs = $this->get_conf_prefs();
					foreach ($confs as $a_conf) {
						if (!is_array($cat_confs[$a_conf[cat_id]])) $cat_confs[$a_conf[cat_id]] = array();
						if (is_array($selected_confs) && in_array($a_conf[uid], $selected_confs)) {
							$cat_confs[$a_conf[cat_id]][] = '<input type="checkbox" checked="checked" name="profile_c[mailer_confs][]" value="'.$a_conf['uid'].'" /> '.$a_conf['conference_name'].'<br />';
						} else {
							$cat_confs[$a_conf[cat_id]][] = '<input type="checkbox" name="profile_c[mailer_confs][]" value="'.$a_conf['uid'].'" /> '.$a_conf['conference_name'].'<br />';
						}
					}
					if (is_array($cat_confs)) {
						foreach ($cat_confs as $cat_uid => $conf) {
							$cat = t3lib_div::makeInstance("tx_chcforum_category",$cat_uid, $this->cObj);
							$out .= '<h1>' . $cat->return_title().'</h1>';
							$out .= implode($conf);
						}
					}
				}
