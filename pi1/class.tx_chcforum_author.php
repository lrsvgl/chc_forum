...
class tx_chcforum_author extends tx_chcforum_user {
	...
	function display () {
		...
		if ($this->conf['cwtCommunityIntegrated'] == true) {
			...
			if ($this->user->uid) {
				$message = t3lib_div::makeInstance("tx_chcforum_message",$this->cObj, tx_chcforum_shared::lang('profile_forum_hdr'), 'profile_hdr_big');
				$out .= $message->display();
	...
	function returnCwtBuddyLink() {
		if( ($this->conf['cwtCommunityIntegrated'] != true) || (!$this->user->uid) ) return;

		$params = array (
			'action' => 'getviewbuddylistadd',
			'buddy_uid' => $this->uid
		);
	...
	function return_total_posts() {
		...
		if ($this->uid) {
			...
			$addWhere = 'post_author='.$GLOBALS['TYPO3_DB']->fullQuoteStr($this->uid,$table);
			$where = tx_chcforum_shared::buildWhere('tx_chcforum_post',$addWhere);
			$qresults = $GLOBALS['TYPO3_DB']->exec_SELECTquery('count(*)',$table,$where);
	...
	function return_img_tag() {
		...
		if (!empty($this->$field) && t3lib_div::validPathStr($this->$field) == true) {
			...
			if (file_exists($img_path.'/'.$img[0]) == true) {
				...
				// file as is.
				if (!$gen_img_path) {
					$out = '<img title="userpic" alt="'.tx_chcforum_shared::lang('alt_userpic').'" class="userPic" src="'.$img_path.'/'.$img[0].'" />';
				} else {
					$out = '<img title="userpic" alt="'.tx_chcforum_shared::lang('alt_userpic').'" class="userPic" src="'.$gen_img_path.'" />';
				}
			}
		}
		return $out;
