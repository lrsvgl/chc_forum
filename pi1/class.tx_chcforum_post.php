	 ...
	 /**
		...
		*/
		function get_threads_new() {
			$user = t3lib_div::makeInstance("tx_chcforum_user");
			$is_new = false;
			$table = 'tx_chcforum_postread';
			$where = "feuser_uid= '" . $user->uid . "' AND post_uid='".$this->uid."'";
			$results = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', $table, $where);

			$data = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($results);
	 ...
	 /**
		...
		*/
		function return_post_info ($view = 'all_cats') {
			switch ($view) {
				case 'all_cats':
				$thread = t3lib_div::makeInstance("tx_chcforum_thread",$this->thread_id, $this->cObj);
	 ...
	 /** 
		...
		*/
		function display_post () {
  		...
  		$this->author = t3lib_div::makeInstance("tx_chcforum_author",$this->post_author, $this->cObj);
	  	
	  	$img_tag = $this->author->return_img_tag();
			 
			if (!$this->tmpl_path) $this->tmpl_path = tx_chcforum_shared::setTemplatePath();
				$tmpl = t3lib_div::makeInstance("tx_chcforum_tpower",$this->tmpl_path.'single_post.tpl');
			$tmpl->prepare();			
			...
			$tmpl->assign('customim_link', $im_links['customim']);
			  ...
			  if ($this->hidden == 1) {
				$message = t3lib_div::makeInstance("tx_chcforum_message",$this->cObj, tx_chcforum_shared::lang('post_hidden'), 'error_no_border');
			...
			if ( ($this->fconf['allow_rating'] != 'no')  && $this->preview != true	) {
				...
				$options = '';
				if( $this->fconf['allow_rating'] == 'yes' || ( ($this->fconf['allow_rating'] == 'logged_in_user') && $this->user->uid) ) {
					...
					// Add submit button					
					$options .= '<input type="submit" name="submit" value="' . tx_chcforum_shared::lang('display_post_rate_submit_label') . '"/>';
				}
		...
		// returns the rating for a post as a rounded int.
		function getRating($dontRound = false) {
			$rating = array();
			if ($this->uid) {
				$table = 'tx_chcforum_ratings';
				$fields = 'AVG(rating) as avg, COUNT(*) as count';
				$addWhere = 'post_uid='.$GLOBALS['TYPO3_DB']->fullQuoteStr($this->uid,$table);
				$where = $addWhere;
	 ...
	 /**
		...
		*/
		function return_post_time() {
			if ($this->crdate) {
				// todo: warum wird hier klasse Instanziert?
...
#			}
		...
		function unhide() {
			if ($this->user->can_mod_conf($this->conference_id)) {
				$data_arr['hidden'] = 0;
				$table = 'tx_chcforum_post';
				$where = 'uid='.$GLOBALS['TYPO3_DB']->fullQuoteStr($this->uid,$table);
